...
export const useGameEngineElectromagnet = ({
  levelData,
  magnets,
  gameStatus,
  containerRef,
}: UseGameEngineProps<'electromagnet'>) => {
      ...
      p.setup = () => {
        ...
        Matter.Events.on(engine, 'beforeUpdate', () => {
          ...
          magnets.forEach((magnet) => {
            if (!magnet.isActive) return; // Skip inactive magnets
      ...
      p.draw = () => {
        ...
        magnets.forEach((magnet) => {
          ...
          for (let i = 0; i < layerRadii.length; i++) {
            if (!magnet.isActive) continue; // Skip inactive magnets
